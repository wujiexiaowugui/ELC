...
def intersect_dicts1(da, db, exclude=()):
    # Dictionary intersection of matching keys and shapes, omitting 'exclude' keys, using da values
    import pdb
    ...
    for k, v in da.items():
        ...
        if 'model.0' in k:
            new_k = k
            if v.shape == db[new_k].shape:
                dict_v[new_k] = v
                
        elif 'bn' in k:
            new_k = k_c.replace('bn','1')
            print(new_k)
            if v.shape == db[new_k].shape:
                dict_v[new_k] = v
                
        elif 'conv' in k:
            new_k = k_c.replace('conv','0')
            print(new_k)
            # dict_v['model.'+k]
            if v.shape == db[new_k].shape:
                dict_v[new_k] = v
    ...
    return dict_v#{'model.'+k: v for k, v in da.items() if 'model.'+k in db and not any(x in k for x in exclude) and v.shape == db['model.'+k].shape}
